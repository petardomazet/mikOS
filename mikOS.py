...
"""     # kasnije ćemo ovo implementirat - ako bude trebalo
...
def system_information():       #System information app
    ...
    bar = Label(sysInf, text=(" " * 100000), fg=windowBarColor, bg=windowBarColor)  # crna traka na vrhu 
    ...
    totalDiskSpace = round(total / (1024.0 ** 3))         # total disk space
    totalDiskSpaceString = str(totalDiskSpace)

    usedDiskSpace = round(used / (1024.0 ** 3))           # used disk space
    usedDiskSpaceString = str(usedDiskSpace)

    freeDiskSpace = (round(free / (1024.0 ** 3)))         # free disk space
    freeDiskSpaceString = str(freeDiskSpace)

    # --- labels ---        
    SystemInformationLabel = Label(sysInf, text="System Information Overview", fg="#F6F6FA", font=("Segoe UI", 22), bg=darkMode)    # title label on top of the window
    SystemInformationLabel.place(x=45, y=27.5)
    hr1 = Label(sysInf, text=("_"*495), fg="grey", bg=darkMode, font=("Helvetica", 2)).place(x=45, y=75)        # horizontal line

    mikOSVersion = Label(sysInf, text="mikOS", fg="#F6F6FA", font=("Franklin Gothic Medium", 18), bg=darkMode)      # release of mikOS
    ...
    copyrightLabel = Label(sysInf, text="© 2022 MiK Corporation. All rights reserved.", fg="#F6F6FA", font=("Segoe UI", 12), bg=darkMode)   # copyright (c) label
    copyrightLabel.place(x=45, y=125)
    
    hr2 = Label(sysInf, text=("_"*495), fg="grey", bg=darkMode, font=("Helvetica", 2)).place(x=45, y=150)   # horizontal line


    hardwareLabel = Label(sysInf, text="Hardware", fg="#F6F6FA", font=("Segoe UI", 14, 'bold'), bg=darkMode)    # subtitle hardware
    hardwareLabel.place(x=45, y=157)

    processorLabel = Label(sysInf, text=("Processor: " + processor1), fg="#F6F6FA", font=("Segoe UI", 12), bg=darkMode) # CPU informations
    processorLabel.place(x=45, y=189)

    InstalledMemoryRAM = Label(sysInf, text=("Installed Memory (RAM): " + RamCapacityRounded + ",00" + " GB"), fg="#F6F6FA", font=("Segoe UI", 12), bg=darkMode)    # ram informations
    InstalledMemoryRAM.place(x=45, y=210)

    totalDiskCapacity = Label(sysInf, text=("Total Disk Capacity: " + totalDiskSpaceString + " GB"), fg="#F6F6FA", font=("Segoe UI", 12), bg=darkMode)  # total disk capacity LABEL
    ...
    '''
    WindowBar = Label(sysInf, text=("" * 350))
    WindowBar.place(x=0, y=0)
    '''
    def exit():
        sysInf.destroy()
    exitSysInfo = Button(sysInf, text="✕", bg="red", borderwidth=0, fg="white", command=exit) #popraviti command
    exitSysInfo.place(x=1060, y=0, height=24, width=40)
